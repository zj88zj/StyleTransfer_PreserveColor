...
#     out = cv2.cvtColor(src_yiq, cv2.COLOR_YCrCb2RGB)
#     out_img = PIL.Image.fromarray(out)

#     return out_img

def transfer_color(content, generated):
    content_channels = list(content.convert('YCbCr').split())
    generated_channels = list(generated.convert('YCbCr').split())
    content_channels[0] = generated_channels[0]
    return PIL.Image.merge('YCbCr', content_channels).convert('RGB')
...
def style_transfer(name, content_image, style_image, image_size, style_size, content_layer, content_weight,
    ...
    plt.axis('off')
    # plt.imshow(deprocess(img.cpu()))
    plt.imshow(transfer_color(deprocess(content_img.cpu()), deprocess(img.cpu())))
